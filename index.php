...
if (file_exists('setup_completed.flag')) {
    ...
    $sqlCreateDB = "CREATE DATABASE IF NOT EXISTS restaurantdb"; //ebookdb
    if ($link->query($sqlCreateDB) === TRUE) {
        echo "Database 'restaurantdb' created successfully.<br>";//ebookdb
    } else {
        echo "Error creating database: " . $link->error . "<br>";
    }

    // Switch to using the 'restaurantdb' database
    $link->select_db('restaurantdb');//ebookdb
    ...
    // Execute SQL statements from "restaurantdb.txt"
    executeSQLFromFile('restaurantdb.txt', $link);//ebookdb
